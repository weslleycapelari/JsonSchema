...
- Includes auxiliary developer tools under `tools/`: SchemaMockGen (data mock generator), Schema2Delphi (Delphi DTO class generator), SchemaValidator (JSON Schema validator), and Delphi2Schema (JSON Schema generator from Delphi classes/records using RTTI). Each tool provides both a Command-Line Interface (CLI) and a Desktop VCL GUI application.
