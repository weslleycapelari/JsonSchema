# SchemaValidator - Command Line Usage

This document describes how to use `SchemaValidator` to validate JSON instance documents against JSON Schemas, including configuration options, exit codes, output formatting, and redirection patterns.
...
`SchemaValidator` returns standard OS exit codes, making it perfect for pipeline integrations:
...
<testsuite name="JSON Schema Validation" tests="1" failures="1" errors="0" time="0.00">
  <testcase name="Validate customer.json" classname="SchemaValidator" time="0.00">
...
```bash
cat instance.json | ./SchemaValidator -s schema.json
```
...
```bash
./SchemaValidator -s schema.json -i instance.json
exit_code=$?
