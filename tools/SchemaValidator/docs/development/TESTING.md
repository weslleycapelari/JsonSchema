# SchemaValidator - Testing Guide

This guide describes how to run and write tests for the `SchemaValidator` utility.

## Testing Strategy

To ensure high reliability, `SchemaValidator` is validated using a two-tier testing approach:
...
2. **Integration Testing**: Spawns the compiled `SchemaValidator.exe` binary as a child process using the Windows API. It validates:
...
All tests are implemented in `tools/SchemaValidator/test/src/TestSchemaValidator.pas` and run under the DUnit framework.

## Running Tests

Before running the tests, compile the production CLI binary first (as described in the [Setup Guide](SETUP.md)), since the integration tests depend on `SchemaValidator.exe`.

### 1. Running Console Tests

To execute tests and view output directly in your shell:

1. Open your console in `tools/SchemaValidator/test/console/`.
...
3. Run the compiled executable:
   ...
   ```powershell
   ./TestSchemaValidatorConsole.exe
   ```
...
1. Open your console in `tools/SchemaValidator/test/gui/`.
