unit TestSchemaValidator;

(*
--------------------------------------------------------------------------------
Integration and unit tests for SchemaValidator.
--------------------------------------------------------------------------------
*)
...
type
  /// <summary>Test suite containing unit and integration tests for SchemaValidator.</summary>
  TTestSchemaValidator = class(TTestCase)
  strict private
    /// <summary>Spawns the SchemaValidator process and captures its exit code and stdout/stderr.</summary>
...
function TTestSchemaValidator.RunCLI(const pArgs: string; out pStdout, pStderr: string): Integer;
var
...
procedure TTestSchemaValidator.CreateTempFile(const pContent: string; out pPath: string);
var
...
procedure TTestSchemaValidator.DeleteTempFile(const pPath: string);
begin
  if (pPath <> '') and FileExists(pPath) then
    System.SysUtils.DeleteFile(pPath);
end;

procedure TTestSchemaValidator.TestParseArguments;
var
  lConfig: TConfig;
begin
...
procedure TTestSchemaValidator.TestAutoDetectDraft;
var
  lJson: TJSONValue;
begin
...
procedure TTestSchemaValidator.TestCLIExecutionSuccess;
var
...
procedure TTestSchemaValidator.TestCLIExecutionFailure;
var
  lSchemaPath, lInstancePath: string;
  lStdout, lStderr: string;
  lExitCode: Integer;
...
procedure TTestSchemaValidator.TestCLIExecutionRuntimeError;
var
  lStdout, lStderr: string;
  lExitCode: Integer;
  lSchemaPath: string;
...
initialization
  RegisterTest(TTestSchemaValidator.Suite);
