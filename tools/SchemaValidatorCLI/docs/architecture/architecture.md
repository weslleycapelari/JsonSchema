# SchemaValidatorCLI - Component Architecture

This document describes the internal structure, dependencies, and execution flow of the `SchemaValidatorCLI` utility.

## Architectural Overview

`SchemaValidatorCLI` is designed with modularity, testability, and separation of concerns in mind. Instead of writing all CLI logic in a single monolithic `.dpr` file, the tool is split into multiple lightweight Pascal units located in the `src/` directory.
...
flowchart TD
    ...
    B --> C[SchemaValidatorCLI.Runner]
    C --> D[SchemaValidatorCLI.Config]
    C --> E[SchemaValidatorCLI.Utils]
    C --> F[JsonSchema.Validator Core]
    C --> G[SchemaValidatorCLI.Formatters]
...
- Very thin wrapper; its only responsibility is calling `RunSchemaValidatorCLI` from the Runner module and catching any unhandled exceptions to return exit code 2.

### 2. Configuration Parser (`SchemaValidatorCLI.Config.pas`)
...
### 3. Utility Helpers (`SchemaValidatorCLI.Utils.pas`)
...
### 4. Output Formatters (`SchemaValidatorCLI.Formatters.pas`)
...
### 5. Orchestrator Runner (`SchemaValidatorCLI.Runner.pas`)
