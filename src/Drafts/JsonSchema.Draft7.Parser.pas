unit JsonSchema.Draft7.Parser;

(*
--------------------------------------------------------------------------------
Provides compilation and parsing capabilities for Draft 7 schemas.
--------------------------------------------------------------------------------
*)
...
type
  /// <summary>Compiler for Draft 7 schemas. Leverages a registry to dynamically register keyword plugins.</summary>
  TDraft7Parser = class
  strict private
...
class constructor TDraft7Parser.Create;
begin
...
class procedure TDraft7Parser.RegisterLogicalKeywords;
begin
...
class destructor TDraft7Parser.Destroy;
begin
  FRegistry.Free;
end;

class function TDraft7Parser.Parse(const pSchema: TJSONObject): ICompiledSchema;
var
...
begin
  ...
  try
    // In Draft 7, if $ref is present, all other keywords (including $id) are ignored.
...
class function TDraft7Parser.ParseSchema(const pSchemaVal: TJSONValue): ICompiledSchema;
begin
  if pSchemaVal is TJSONObject then
    Result := TDraft7Parser.Parse(TJSONObject(pSchemaVal))
